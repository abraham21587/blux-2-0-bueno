...
export class PerfilService {
  ...
  async crearPerfil(nombre: string, avatarUrl: string, usuarioCorreo: string, esMenor?: boolean, pin?: string) {
    if (!nombre || !usuarioCorreo) throw new BadRequestException('Nombre y correo son requeridos.');
    const cantidad = await this.perfilModel.countDocuments({ usuarioCorreo: usuarioCorreo.toLowerCase() });
    if (cantidad >= 5) throw new BadRequestException('Máximo 5 perfiles por usuario.');
  ...
  async crearLote(perfiles: any[]) {
    ...
    if (perfiles.length > 5) throw new BadRequestException('Máximo 5 perfiles por lote.');
    return this.perfilModel.insertMany(perfiles.map(p => ({ ...p, usuarioCorreo: p.usuarioCorreo?.toLowerCase() })));
  }

  async editarPerfil(id: string, nombre?: string, avatarUrl?: string, pin?: string) {
